...
# IMAGE CRAWLER
from autotag.img.crawlers.crawler import Crawler
...
def crawl_img(
    ...
    cfg is a list of [{
        ...
        'loc' : location where the image should be saved (ex: user/downloads ; NOT user/downloads/keyword),
        'ext' : The extension
    })
    ...
    for elem in keywords:
        ...
        # changing target_dir to the folder for current folder so that earlier augmented images(in prev keyword repo) are not augmented again
        # target_dir = os.path.join(target_dir, keyword.replace('"', ""))

        # Augmentation part commented out for now (functional when uncommented)
        ...
        #         if transform_func is not None: # apply transform function to each image generated (if provided)
        ...
        #         img_path_main = os.path.join(os.path.dirname(img_path), f'{fcounter:05}')
        #         print(img_path_main)
...
if __name__ == "__main__":
    ...
    # For now
    # max : 0  means download all available images from both browsers
    # set target_loc to the download folder eg: user/downloads/keyword.
