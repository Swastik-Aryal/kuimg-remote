...
def autotag_img_fetch(
    ...
    for _ in keywords : 
        ...
        if len(max_specifier) > 1 : 
            raise HTTPException(detail = f'Cannot have multiple count specifiers : {_}', status_code=400)
        try : 
           if len(max_specifier) == 1 : 
            ...
            raise HTTPException(detail = f'count specifier must be integer : {_}', status_code=400)
...
async def train(
    ...
    tag : str = Body(
        ...,
        description = "tag for which the dataset is provided"
        ),
    ) : 
    """
...
dataset_key : str = Body(
    ...
    batch_size = config.GAN_BATCH_SIZE # reducing batch size becuase of lesser number of data
    ...
    try:
        ...
        image_save_path = os.path.join(config.TEMP_AUTO_IMG_PATH,f"{name}_dataset.data")
        ...
        save_with_other_dataset(Path(dataset_temp_loc), Path(new_images_path), tag=name,key=dataset_key_GAN)
...
async def model_register(
    ...
    zip_loc = os.path.join(config.TEMP_DATA_PATH, model_key,)
    ...
    if not os.path.exists(unzip_loc) : os.makedirs(unzip_loc)
